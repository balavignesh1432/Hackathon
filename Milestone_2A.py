...
def timeFunc(parent, times):
    if times['FunctionInput'][0] != '$':
        ...
        currTime = datetime.datetime.now()
        string = str(currTime) + ';' + parent + ' Executing TimeFunction(' + shared[times['FunctionInput'][2:len(times['FunctionInput']) - 1]] + ', ' + times[
            'ExecutionTime'] + ')'
        logs.append(string)
        time.sleep(int(times['ExecutionTime']))
...
def dataLoad(parent, data):
    ...
    shared[parent] = {"DataTable":dataTable, "NoOfDefects":len(dataTable)}
    file.close()
